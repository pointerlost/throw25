...
namespace LIGHTING
{
	...
	public:
		...
		void update(const std::shared_ptr<SCENE::SceneObject>& lightObject) const;

		void setLightType(const LightType type) noexcept { m_type = type; };
		[[nodiscard]] LightType getType() const { return m_type; };

		void setVisualObject(std::shared_ptr<SCENE::SceneObject> lightObject) { m_visual = lightObject; };
		[[nodiscard]] std::weak_ptr<SCENE::SceneObject> getVisual() const;

		void setLightData(const std::shared_ptr<LightData>& lightData) { m_lightData = lightData; };
	...
	private:
		...
		std::weak_ptr<SCENE::SceneObject> m_visual;
